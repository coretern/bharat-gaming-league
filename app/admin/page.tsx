...
export default function AdminPanel() {
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-24 pb-12 font-bold">
      ...
      {/* Enhanced Rejection Suite Modal */}
      {rejectingId && viewReg && (
          <div className="fixed inset-0 z-[110] bg-black/60 backdrop-blur-md flex items-center justify-center p-4" onClick={() => setRejectingId(null)}>
              <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2.5rem] shadow-2xl p-8 relative overflow-hidden animate-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                  ...
                  <button onClick={() => setRejectingId(null)} className="absolute top-8 right-8 p-2.5 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-500 hover:text-red-500 transition-all active:scale-90">
                      <X className="w-5 h-5" />
                  </button>
                  ...
                  <div className="flex gap-4 mt-10">
                      <button onClick={() => setRejectingId(null)} className="flex-1 h-14 rounded-2xl bg-slate-100 dark:bg-slate-800 text-slate-500 font-bold text-xs uppercase hover:bg-slate-200 transition-all">
                          Cancel
                      </button>
                      <button 
                        onClick={() => {
                            ...
                            updateStatus(viewReg._id, { 
                                status: 'Rejected', 
                                rejectionReason: finalMsg || 'Registration rejected due to data issues.',
